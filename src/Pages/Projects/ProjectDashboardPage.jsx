...
export function ProjectDashboardPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (activeTab === 'sales') {
      ...
      const paid = form.sales.amount_paid !== '' ? Number(form.sales.amount_paid) : (form.sales.payment_mode === 'CREDIT_LEDGER' ? 0 : saleTotal);
      const balanceDue = Math.max(0, saleTotal - paid);

      if (balanceDue > 0 && !customerData) {
        alertModal(`A customer must be attached when the amount paid (Ksh ${paid.toLocaleString()}) is less than the required total amount (Ksh ${saleTotal.toLocaleString()}). Outstanding balance: Ksh ${balanceDue.toLocaleString()}`, 'warning');
  ...
  const renderRecordMeta = (entry) => {
    ...
    if (activeTab === 'sales') {
      ...
      if (bal > 0) {
        return `Ksh ${total.toLocaleString()} (Due: Ksh ${bal.toLocaleString()})`;
      }
      return `Ksh ${total.toLocaleString()} (Paid)`;
    }
  ...
  return (
    <div className={styles.page}>
          ...
          {showRecordForm && (
            <form onSubmit={handleSubmit} className={styles.recordForm}>
              ...
              {activeTab === 'sales' && (
                <>
                  <label>
                    <span>Item to Sell <span className={styles.requiredStar}>*</span></span>
                    <input 
                      list="harvest-produce-options"
                      name="item" 
                      value={form.sales.item} 
                  ...
                  {harvestStock.length > 0 && (
                    <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginTop: '-0.3rem', marginBottom: '0.75rem' }}>
                      <span style={{ fontSize: '0.75rem', color: '#94a3b8', alignSelf: 'center' }}>Harvested Produce:</span>
                      {harvestStock.map((hs) => (
                        <button
                          key={hs.item}
                          type="button"
                          onClick={() => setForm((prev) => ({ ...prev, sales: { ...prev.sales, item: hs.item } }))}
                          style={{
                            padding: '0.22rem 0.6rem',
                            fontSize: '0.78rem',
                            borderRadius: '6px',
                            background: form.sales.item?.toLowerCase() === hs.item.toLowerCase() ? 'rgba(16, 185, 129, 0.25)' : 'rgba(255, 255, 255, 0.06)',
                            border: form.sales.item?.toLowerCase() === hs.item.toLowerCase() ? '1px solid #10b981' : '1px solid rgba(255, 255, 255, 0.12)',
                            color: form.sales.item?.toLowerCase() === hs.item.toLowerCase() ? '#34d399' : '#e2e8f0',
                            cursor: 'pointer',
                            display: 'inline-flex',
                            alignItems: 'center',
                            gap: '0.35rem'
                  ...
                  <div className={styles.formRow}>
                    <label style={{ flex: 1 }}><span>Quantity <span className={styles.requiredStar}>*</span></span><input type="number" step="0.01" min="0.01" name="quantity" value={form.sales.quantity} onChange={handleFieldChange} placeholder="20" required /></label>
                    <label style={{ flex: 1 }}><span>Unit price (Ksh) <span className={styles.requiredStar}>*</span></span><input type="number" step="0.01" min="0.01" name="unit_price" value={form.sales.unit_price} onChange={handleFieldChange} placeholder="75" required /></label>
                  </div>
                  ...
                  <div className={styles.formRow}>
                    <label style={{ flex: 1 }}>
                      ...
                      <select
                        name="payment_mode"
                        value={form.sales.payment_mode || 'CASH'}
                        onChange={(e) => {
                          const mode = e.target.value;
                          const total = (Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0);
                          setForm((prev) => ({
                            ...prev,
                            sales: {
                              ...prev.sales,
                              payment_mode: mode,
                              amount_paid: mode === 'CREDIT_LEDGER' ? '0' : (prev.sales.amount_paid || (total > 0 ? total.toString() : ''))
                            }
                          }));
                        }}
                      >
                    ...
                    <label style={{ flex: 1 }}>
                      ...
                      <input
                        ...
                        min="0"
                        name="amount_paid"
                        value={form.sales.amount_paid}
                        onChange={handleFieldChange}
                        placeholder={form.sales.payment_mode === 'CREDIT_LEDGER' ? '0' : ((Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0) || '0').toString()}
                      />
                    </label>
                  </div>

                  {Number(form.sales.quantity) > 0 && Number(form.sales.unit_price) > 0 && (
                    <div style={{ padding: '0.75rem 1rem', background: '#0f172a', borderRadius: '0.5rem', border: '1px solid #334155', marginBottom: '1rem', fontSize: '0.88rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                        <span style={{ color: '#94a3b8' }}>Total Sale Amount:</span>
                        <strong style={{ color: '#f8fafc' }}>
                          Ksh {((Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0)).toLocaleString()}
                        </strong>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                        <span style={{ color: '#94a3b8' }}>Amount Paid Now:</span>
                        <span style={{ color: '#10b981', fontWeight: 600 }}>
                          Ksh {(Number(form.sales.amount_paid !== '' ? form.sales.amount_paid : (form.sales.payment_mode === 'CREDIT_LEDGER' ? 0 : (Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0)))).toLocaleString()}
                        </span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '0.25rem', borderTop: '1px dashed #334155' }}>
                        <span style={{ color: '#94a3b8' }}>Recorded to Customer Debt (AR):</span>
                        <strong style={{ color: Math.max(0, ((Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0)) - (Number(form.sales.amount_paid !== '' ? form.sales.amount_paid : (form.sales.payment_mode === 'CREDIT_LEDGER' ? 0 : (Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0))))) > 0 ? '#f59e0b' : '#10b981' }}>
                          Ksh {Math.max(0, ((Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0)) - (Number(form.sales.amount_paid !== '' ? form.sales.amount_paid : (form.sales.payment_mode === 'CREDIT_LEDGER' ? 0 : (Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0))))).toLocaleString()}
                        </strong>
                      </div>
                    </div>
                  )}
                  ...
                  {(() => {
                    const currentTotal = (Number(form.sales.quantity) || 0) * (Number(form.sales.unit_price) || 0);
                    const currentPaid = form.sales.amount_paid !== '' ? Number(form.sales.amount_paid) : (form.sales.payment_mode === 'CREDIT_LEDGER' ? 0 : currentTotal);
                    const currentBalanceDue = Math.max(0, currentTotal - currentPaid);
                    const isCustomerMandatory = currentBalanceDue > 0 || form.sales.payment_mode === 'CREDIT_LEDGER';

                    return (
                      <div className={styles.customerSectionCard} style={{
                        borderColor: isCustomerMandatory && !customerData ? '#ef4444' : undefined,
                        boxShadow: isCustomerMandatory && !customerData ? '0 0 0 1px rgba(239, 68, 68, 0.25)' : undefined
                      }}>
                        ...
                        {customerData ? (
                          ...
                          <div className={styles.emptyCustomerBox}>
                            <p className={styles.emptyCustomerText} style={{ color: isCustomerMandatory ? '#f87171' : undefined }}>
                              {isCustomerMandatory ? (
                                <>⚠️ <strong>Customer is required</strong> because amount paid is less than total required amount (Ksh {currentBalanceDue.toLocaleString()} balance due).</>
                              ) : (
              ...
              {activeTab === 'harvest' && (
                <>
                  ...
                  <div className={styles.formRow}>
                    <label style={{ flex: 1 }}><span>Quantity</span><input type="number" step={0.01} min={0} name="quantity" value={form.harvest.quantity} onChange={handleFieldChange} placeholder="150" required /></label>
                    <label style={{ flex: 1 }}><span>Units</span><input type="text" name="units" value={form.harvest.units} onChange={handleFieldChange} placeholder="kg / crates / litres" required /></label>
                  </div>
                      ...
                      <div className={styles.recordActionGroup}>
                        {activeTab === 'activities' && (
                          <button
                            ...
                            onClick={() => setSelectedActivityForLabor(entry)}
      ...
      {/* Activity Labor Assignment Modal */}
